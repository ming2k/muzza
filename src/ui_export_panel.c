...
void ui_draw_export_panel(fx_canvas* canvas, muzza_ui_state* state, muzza_ui_actions* actions, int win_w, int win_h) {
    ...
    float panel_x = 0.0f;
    float panel_y = 0.0f;
    float close_x = panel_w - 42.0f * s;
    float cancel_x = 18.0f * s;
    float export_x = panel_w - 116.0f * s;
    ...
    /* Export button (becomes CLOSE when finished) */
    ...
    ui_draw_rect(canvas, export_x, panel_y + panel_h - 46.0f * s, 98.0f * s, 28.0f * s, export_btn_bg);
    ui_draw_border(canvas, export_x, panel_y + panel_h - 46.0f * s, 98.0f * s, 28.0f * s, s, export_btn_border);
    ...
    if (state->input.left_pressed) {
        ...
        /* Export / Close button */
        if (is_done && ui_point_in_rect(state->input.x, state->input.y,
